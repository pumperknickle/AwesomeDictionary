...
final class MappingSpec: QuickSpec {
    override func spec() {
        let newMap = MappingAlt<String, [[String]]>()
        let key1 = "foo"
        let value1 = [["fooValue"]]
        let key2 = "bar"
        let value2 = [["barValue"]]
...
//            let data6 = try! JSONEncoder().encode(map6)
            it("should delete correctly") {
                for _ in 0...100000 {
                    let keyToTest = "\(Int.random(in: 0...100000))"
                    expect(map4.setting(key: keyToTest, value: [[keyToTest]]).setting(key: keyToTest + "1", value: [[keyToTest]]).deleting(key: keyToTest).contains(keyToTest)).to(beFalse())
                }
            }
...
1				expect(overwritten.keys()).to(contain([key1, key2, key3, key4]))
